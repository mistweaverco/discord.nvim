...
{ 
    'mistweaverco/discord.nvim',
    event = "VeryLazy"
    -- All available config options with their default values:
    opts = {
      -- General options
      ...
      plugins_text        = "Using %s",                 -- Format string rendered when using a plugin
      plugin_manager_text = "Managing plugins",         -- Format string rendered when managing plugins (either string or function(plugin_manager_name: string): string)
      jujutsu_text        = {                           -- Format strings for jujutsu.nvim views (or function(presence): string)
      ...
      -- Other options
      silence_discord_socket_errors = false,            -- Whether to silence errors from the Discord IPC socket (e.g., when Discord is not running)
  }
}
```
